...
(() => {
    ...
    function updateMenuAuthItem() {
        if (!dom.menuLogin) return;
        const icon = dom.menuLogin.querySelector('.material-icons-round');
        const label = dom.menuLogin.querySelector('[data-i18n]');
        const authenticated = Sharing.isAuthenticated();
        if (icon) {
            icon.textContent = authenticated ? 'logout' : 'login';
        }
        if (label) {
            label.textContent = authenticated ? Lang.t('menuLogout') : Lang.t('menuLogin');
            label.dataset.i18n = authenticated ? 'menuLogout' : 'menuLogin';
        }

        updateMenuAccessState(authenticated);
    }

    function setMenuItemAvailability(element, isActive) {
        if (!element) return;
        element.disabled = !isActive;
        element.setAttribute('aria-disabled', String(!isActive));
    }

    function updateMenuAccessState(authenticated) {
        if (dom.sideMenu) {
        ...
        if (dom.menuAuthSubtitle) {
            dom.menuAuthSubtitle.textContent = authenticated
                ? Lang.t('menuAuthMemberSubtitle')
                : Lang.t('menuAuthGuestSubtitle');
            dom.menuAuthSubtitle.dataset.i18n = authenticated
                ? 'menuAuthMemberSubtitle'
                : 'menuAuthGuestSubtitle';
        }

        setMenuItemAvailability(dom.menuProfile, authenticated);
        setMenuItemAvailability(dom.menuSaved, authenticated);
        setMenuItemAvailability(dom.menuEnterCode, authenticated);
        setMenuItemAvailability(dom.menuTheme, authenticated);
        setMenuItemAvailability(dom.menuLang, authenticated);
    }
    ...
    async function handleSavePin() {
        ...
        const authReady = await ensureAuthenticated();
        if (!authReady) return;
    ...
    async function saveWithName(customName) {
        if (!selectedLocation) return;
        ...
        try {
            const success = await Sharing.saveLocation(lat, lng, finalName, note);
            if (success) {
                updateSaveButton(lat, lng);
                updatePinBadge();
                loadSavedMarkers();
            ...
            showToast(error?.message || 'Konum kaydedilemedi', 'error');
        }
    }
    ...
    async function handleSaveEdit() {
        ...
        try {
            await Sharing.updateLocation(editingLocationId, newName, newNote);
            ...
            showToast(error?.message || 'Konum güncellenemedi', 'error');
        }
    }
    ...
    async function handleShareCode() {
        ...
        try {
            const result = await Sharing.createShareCode(selectedLocation.lat, selectedLocation.lng, selectedLocation.name);
            ...
            showToast(error?.message || 'Kod oluşturulamadı', 'error');
        }
    }
    ...
    async function handleFindCode() {
        const code = Array.from(dom.codeDigits).map(i => i.value).join('');
        if (code.length < 6) return;

        let result = null;
        try {
            result = await Sharing.lookupCode(code);
        } catch (error) {
            showToast(error?.message || 'Kod sorgulanamadı', 'error');
            return;
        }
        ...
        if (result) {
            // Found from backend or local fallback
            ...
            dom.codeResult.innerHTML = `
                ...
                <h4>Kod Bulunamadı</h4>
                <p>Girilen kod geçersiz, süresi dolmuş veya henüz oluşturulmamış olabilir.</p>
            `;
        }
    }
    ...
    async function openSavedLocations() {
        ...
        try {
            await Sharing.refreshSavedLocations();
        } catch (error) {
            showToast(error?.message || 'Kayıtlı konumlar alınamadı', 'error');
            return;
        }
        ...
        dom.savedList.querySelectorAll('.saved-item-actions .icon-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                ...
                switch (action) {
                    ...
                    case 'delete':
                        try {
                            await Sharing.removeLocation(id);
                            ...
                            showToast(error?.message || 'Konum silinemedi', 'error');
                        }
                        break;
                }
            });
    ...
    async function ensureAuthenticated() {
        if (Sharing.isAuthenticated()) {
            return true;
        }
        showToast('Bu işlem için giriş yapmalısın', 'login');
    ...
    function bindEvents() {
        ...
        // Menu items
        dom.menuSaved.addEventListener('click', async () => {
            closeSideMenu();
            setTimeout(() => {
                openSavedLocations();
            }, 350);
    ...
    async function init() {
        ...
        try {
            currentUser = await Sharing.syncCurrentUser();
            if (currentUser) {
                await Sharing.refreshSavedLocations();
            }
        } catch {
